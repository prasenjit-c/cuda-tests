...
using functionDef = void (*)(long n, float *x, float *y, float *user_arg);
...
__global__
void fma(long n, T *x, T *y, float *user_arg)
{
...
int main(int argc, char *argv[])
{
  if(argc < 4)
  {
    std::cerr << "Usage: ./ami_test Mem-Size-in-KB Num-Blocks Block-Size Num-FMAs <Val-1> <Val-2> ...\n";
    return -1;
  }
  ...
  std::cout << "Memory Targetted: " << Mem_Size/(1024*1024) << " MB" << std::endl;
  ...
  std::cout << "Memory Address of Load(X): 0x" << std::hex << x << std::endl;
  std::cout << "Memory Address of Store(Y): 0x" << std::hex << y << std::endl;
  ...
  run_fma_kernel[numFMAs-1]<<<numBlocks, blockSize>>>(N, x, y, user_arg);
