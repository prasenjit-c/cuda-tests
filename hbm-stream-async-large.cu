...
constexpr int async_xfer_size = 128;
...
__global__ void init_kernel(T *A, const size_t N)
{
  ...
  int scaled_element = async_xfer_size/sizeof(T);

  __syncthreads();
  for(int i = tidx * scaled_element; i < N; i += stride * scaled_element)
  {
     cuda::memcpy_async(&A[i], &sh_mem_buf[threadIdx.x * scaled_element], cuda::aligned_size_t<16>(scaled_element * sizeof(T)), *bar);
  }
  __syncthreads();
...
__global__ void read_kernel(T *A, const size_t N)
{
  ...
  for(int i = tidx * scaled_element; i < N; i += stride * scaled_element)
  {
     cuda::memcpy_async(&sh_mem_buf[threadIdx.x * scaled_element], &A[i], cuda::aligned_size_t<16>(scaled_element * sizeof(T)), *bar);
  }
  __syncthreads();
//  bar->arrive_and_wait(); // Wait for all copies to complete
}
...
int main(int argc, char **argv)
{
  ...
  else
  {
	...
	sleep(2);
	for (blockSize = 1; blockSize <= MAXWarpSize; blockSize++)
	   measureKernels(kernels, blockSize, gridSize, 2);
